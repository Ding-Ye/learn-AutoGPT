...
//
// s03 is where this 8-profile table earns its place in every later
// session's main.go. Read alongside provider_openai.go to see how a
// single Anthropic-shape internal model serves all 7 OpenAI-compat
// destinations through one wire-format translation layer.
...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(flag.CommandLine.Output(),
			"usage: s03-llm-provider [-v] [-provider P] [-base-url URL] [-model ID] <prompt>\n\n"+
				...
				"  Examples:\n"+
				"    s03-llm-provider -v \"add 2 and 3\"                  # default Anthropic\n"+
				"    s03-llm-provider -provider deepseek -v \"echo hi\"   # DeepSeek\n"+
				"    s03-llm-provider -provider deepseek -model deepseek-reasoner \"...\"\n"+
				"    s03-llm-provider -provider local -model llama-3.3 \"...\"\n")
	}
	flag.Parse()
	...
	if *verbose {
		fmt.Fprintf(os.Stderr, "[s03-llm-provider] provider=%s model=%s url=%s tools=%d\n",
