...
// TestRegistry_LoopDispatchesViaRegistry — the integration test. In s08
// the Registry is not a direct Loop field anymore; it's synthesized by
// the ComponentBus. We adapt this test to wrap the two tools in a
// commandsOnlyComponent so the bus produces the same Registry shape.
func TestRegistry_LoopDispatchesViaRegistry(t *testing.T) {
	const toolUseID = "toolu_math_01"
	bus := NewComponentBus(&commandsOnlyComponent{tools: []Tool{NewEchoTool(), NewMathTool()}})
	p := NewMockProvider(
	...
	loop := &Loop{Provider: p, Components: bus, MaxTurns: 5}
	...
	// Second request must contain a tool_result whose content is "5".
	// In s05 the tool_result is no longer at messages[len-1] (the trailing
	// user-task message took that slot); we walk the rendered history
	// looking for any tool_result content. The synthesized ToolUseID
	// also no longer matches the original toolUseID — RenderMessages
	// fabricates stable per-episode ids since ActionProposal doesn't
	// carry the wire id forward (deliberate; see history.go).
	_ = toolUseID
