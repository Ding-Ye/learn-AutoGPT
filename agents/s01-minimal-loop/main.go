...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(flag.CommandLine.Output(),
			"usage: s01-minimal-loop [-v] [-provider P] [-base-url URL] [-model ID] <prompt>\n\n"+
				...
				"  Examples:\n"+
				"    s01-minimal-loop -v \"hello\"                          # default Anthropic\n"+
				"    s01-minimal-loop -provider deepseek -v \"hello\"      # DeepSeek\n"+
				"    s01-minimal-loop -provider deepseek -model deepseek-reasoner \"...\"\n"+
				"    s01-minimal-loop -provider local -model llama-3.3 \"...\"\n")
	}
	flag.Parse()
	...
	if *verbose {
		fmt.Fprintf(os.Stderr, "[s01-minimal-loop] provider=%s model=%s url=%s\n", *provider, model, url)
	}
