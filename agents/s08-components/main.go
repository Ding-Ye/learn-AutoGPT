...
// Provider profiles — same surface as s07. The s08 change is in how
// tools are constructed: instead of explicit `reg.Register(...)` calls
// the binary builds a `[]Component` list and wraps it in a
// ComponentBus, which the Loop consumes directly.
...
// permissionsConfig is the on-disk shape we (un)marshal.
...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(flag.CommandLine.Output(),
			"usage: s08-components [-v] [-provider P] [-strategy S] [-workspace DIR] [-permissions FILE] [-ask MODE] [-web-timeout D] <prompt>\n\n"+
				"  s08 wires capabilities through Components instead of explicit Register() calls:\n"+
				...
				"  Provider profiles, strategies, and workspace flags: see s06–s07.\n\n"+
				"  Example:\n"+
				"    s08-components -v \"fetch https://example.com and write a one-line summary to notes.md\"\n")
	}
	flag.Parse()
	...
	// THE s08 SHAPE: a list of components. The bus wraps them, and the
	// Loop consumes the bus directly. No explicit Registry construction
	// in main — the bus synthesizes one from each component's Commands().
	...
	if *verbose {
		fmt.Fprintf(os.Stderr,
			"[s08-components] provider=%s model=%s url=%s strategy=%s components=%d tools=%d directives=%d workspace=%s permissions=%s ask=%s web-timeout=%s\n",
			*provider, model, url, *strategyFlag,
			len(bus.Components()), len(bus.Registry().All()), len(bus.Directives()),
			ws.Root(), sourceLabel, *askMode, webTimeout.String())
	}
	...
	if *verbose {
		fmt.Fprintf(os.Stderr, "[s08-components] final history: %d episodes\n", len(*history))
	}
	fmt.Println(final)
}

// loadPermissions reads `path` as JSON {"allow":[],"deny":[]}.
...
// defaultPermissions: allow the three component-supplied tools without
// args inspection, no deny rules.
